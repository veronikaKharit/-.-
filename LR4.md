...
Проект реализован по модульному принципу. Основная логика разделена на несколько частей: API-слой, модели данных, бизнес-логика, хранилище и тесты. Такая структура делает код более понятным и удобным для сопровождения.

### Структура проекта

- `app/main.py` — точка входа в приложение и описание REST API;
- `app/models.py` — модели запросов и ответов;
- `app/services.py` — бизнес-логика работы со сценариями, сессиями и оценкой ответов;
- `app/storage.py` — хранение данных в памяти;
...
#### `main.py`
В этом файле создается приложение FastAPI и описываются основные маршруты API.  
Файл отвечает только за обработку HTTP-запросов и передачу управления в сервисный слой.

#### `models.py`
Содержит Pydantic-модели для валидации входных данных и формирования ответов.  
Использование моделей позволяет контролировать структуру данных и снижает вероятность ошибок при работе с API.
...
#### `storage.py`
Реализует упрощенное хранилище данных в оперативной памяти.  
Для лабораторной работы этого достаточно, так как основная цель — показать архитектуру и работу API.

### Логика работы системы

Сервис поддерживает следующий сценарий:
1. создание сценария тренировки;
2. создание тренировочной сессии;
3. отправка ответа пользователя;
4. оценка ответа;
5. завершение сессии.

Таким образом, система моделирует базовую работу диалогового тренажера.
...
- проект легко расширить, например, добавив базу данных или более сложную оценку ответов.

### Тестирование

Для проверки работы проекта использовались:
- интеграционные тесты `pytest`;
- коллекция Postman.

Тестирование охватывает проверку доступности сервиса, создание сценария, создание сессии, отправку ответа и завершение сессии.
